...
export async function fetchParkEvents(
  date: Date = new Date(),
  targetKey?: string
): Promise<CalendarInstance[]> {
  // Park Theatre API needs a date range, so we'll fetch a week around the target date
  ...
  const response = await fetch(
    `https://www.theparktheatre.ca/wp-json/barker/v1/listings?_embed=true&end_date=${endStr}&page=1&per_page=500&start_date=${startStr}&status=publish`
  );
